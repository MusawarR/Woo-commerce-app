...
const Registration = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (validate()) {
            try {
                const apiResponse = await RegisterUser(userData.email, userData.password)
        ...
        <div className="w-full min-h-screen flex flex-1 flex-col justify-center items-center space-y-7 bg-slate-50 p-7">
            <div className="space-y-4 p-10 flex flex-col border-[1px] border-gray-300 shadow-md min-w-[450px] rounded-md overflow-clip bg-white">
                ...
                <div className="flex flex-col space-y-1">
                    <p className="font-bold text-gray-600 text-2xl">Register</p>
                    ...
                    <p className="mt-7 text-sm text-gray-700 text-center">Have an account? <Link to="/login" className="text-blue-700 underline">Login</Link></p>
                </div>
            </div>
        </div>
    )
